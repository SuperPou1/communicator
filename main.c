...
int main(int argc, char *argv[]) {
	...
	if (!file || strstr("login", argv[1])) {
		...
		islogin = 1;
		strcpy(server, "http://100.125.92.92:8000");
		...
		json_object_object_add(commandjson, "Command", json_object_new_string(argv[1]));
	}
	...
	printf("Request JSON: %s\n", json_object_to_json_string(commandjson));
	...
	json_object_object_add(responsejson, "Server", json_object_new_string(server));
	printf("Welcome to communicator!\n%s\n", json_object_to_json_string_ext(responsejson, JSON_C_TO_STRING_SPACED | JSON_C_TO_STRING_PRETTY));
